...
describe("TokenMaster", () => {
  ...
  describe("Deployment", () => {
    it("Setting the name", async () => {
      let name = await tokenMaster.name();
      expect(name).to.equal(NAME);
    });

    it("Setting the symbol", async () => {
  ...
  describe("Occasions", () => {
    it("Updating occasion count", async () => {
      const occasionId = await tokenMaster.occasionId();
      expect(occasionId).to.be.equal(1);
    });
  ...
  describe("Minting", () => {
    ...
    it("Updating the number of tickets available", async () => {
      const occasion = await tokenMaster.getOccasion(1);
    ...
    it("Updates seat status", async () => {
      const owner = await tokenMaster.seatToPerson(ID, SEAT);
